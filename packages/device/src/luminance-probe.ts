...
type Stop = { readonly at: number; readonly color: string; readonly token: string }
...
function bodyTargets(
  ...
  return stops.map((stop) => {
    ...
    if (Math.abs(dy) <= wheel.outerR + inset) {
      const reach = Math.sqrt(Math.max(0, (wheel.outerR + inset) ** 2 - dy ** 2))
      x = Math.max(x, reach)
    }
...
function steelTargets(options: TargetOptions): Array<ProbeTarget> {
  ...
  const length = Math.abs(body.width * Math.sin(theta)) + Math.abs(body.height * Math.cos(theta))
  ...
  return STEEL_STOPS.map((stop, index) => {
    ...
    const reach = silhouetteHalfWidth(point.y, halfW, halfH, body.cornerR - inset, body.exponent)
...
function chordMidpoint(
  ...
  const push = (x: number, y: number) => {
    if (Math.abs(x) <= halfW + 1e-6 && Math.abs(y) <= halfH + 1e-6) hits.push({ x, y })
  }
  if (Math.abs(uy) > 1e-9) {
...
export function evaluate(readings: ReadonlyArray<ProbeReading>): Array<ProbeResult> {
