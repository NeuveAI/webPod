...
export function createCoverGlassMaterial(
  ...
  const material = new MeshPhysicalMaterial({ ...params, opacity: 1, transparent: false, envMap })
  ...
  material.customProgramCacheKey = () => `webpod-cover-glass-${size.width}x${size.height}`
  return material
}

export function patchGlassShader(shader: CompilableShader, size: { width: number; height: number }): void {
