import {Group,PerspectiveCamera,Vector2,Matrix4,Vector3} from '/Users/vinicius/code/webPod/packages/device/node_modules/three';
...
for(const r of old.results.filter(r=>r.face==='front')){
 ...
 const path=(await Bun.file(import.meta.dir+'/inverse-path.json').json()).find(q=>q.name===r.viewport.width+'-'+r.face).path;const first=path.findIndex(t=>t.admitted&&!t.center),previous=path[first-1],targetStep=path[first];let{x,y}=previous.center;const trace=[],origin=new Vector3().setFromMatrixPosition(new Matrix4().fromArray(r.matrices.cameraWorld)).applyMatrix4(new Matrix4().fromArray(r.matrices.contentWorld).invert());
 for(let step=0;step<=128;step++){const f=step/128,pointer={x:previous.point.x+(targetStep.point.x-previous.point.x)*f,y:previous.point.y+(targetStep.point.y-previous.point.y)*f},target={x:pointer.x-offset.x,y:pointer.y-offset.y};let determinant=0,error=Infinity,done=false;
 ...
 const cage=wrap.cornerCage((.5-x)*330,(.5-y)*552,{width,height,angle:0});const materialPoint=sampleStickerSurfaceGrid(cage.point,width,height,0,u,v),shellHit=collider.castSegment(origin,materialPoint);trace.push({step,pointer,center:{x,y},error,determinant,done,materialPoint:materialPoint.toArray(),shellGap:shellHit?origin.distanceTo(materialPoint)-shellHit.distance:null,visible:shellHit===null||origin.distanceTo(materialPoint)-shellHit.distance<.01,N:cage.diagnostics.worstDirection?.N});if(!done)break;
 }
 results.push({name:r.viewport.width+'-'+r.face,trace,scope:'128substeps over first failed production interval, localNewton max1modelunit/iteration,16iterations,12backtracks, no distantseed. Diagnostic only.'});
}
await Bun.write(import.meta.dir+'/continuation-128.json',JSON.stringify(results,null,2));console.log(results.map(r=>({name:r.name,last:r.trace.at(-1)})));collider.dispose();shell.dispose();
