...
export function StickerEditor({ screen, quad, contour, beginTransform, place, returnToPack }: {
  ...
  useLayoutEffect(() => {
    if (activeId.current !== selectedId) { activeId.current = selectedId; tooltipDismissedAt=null;tooltipPointer=null;deviceStore.set(releaseAtom, null); deviceStore.set(layoutAtom, null); deviceStore.set(tooltipAtom, null) }
  }, [selectedId])
  const cancel = (): void => {
  ...
  const handle = (point: { x: number; y: number }, index: number) => <button key={index} type="button" data-hud-handle="rotate" data-contour-corner={index} aria-label={`Rotate sticker, ${['top left', 'top right', 'bottom right', 'bottom left'][index]} corner`} {...tipEvents(`corner-${index}`, 'Drag to rotate')} className={`${buttonClass} contour-grip`} style={{ position: 'fixed', left: point.x - 22, top: point.y - 22, touchAction: 'none', cursor: 'grab', transform: `scale(${.8 + presence * .2})` }} aria-disabled={state === null || state.phase === 'saving'} onPointerDown={event => begin(event, 'rotationDeg', index)} onPointerMove={event=>{move(event);tipEvents(`corner-${index}`,'Drag to rotate').onPointerMove(event)}} onPointerUp={finish} onPointerCancel={cancel} onLostPointerCapture={() => { if (drag.current !== null) cancel() }} onKeyDown={event => keys(event, 'rotationDeg')} onKeyUp={event => { if (event.key !== 'Escape') commitKey() }}><svg aria-hidden="true" viewBox="0 0 32 32" width="28" height="28" style={{ transform: `translate(${(visibleGrips[index]?.x ?? point.x)-point.x}px,${(visibleGrips[index]?.y ?? point.y)-point.y}px)`, pointerEvents: 'none' }}><g transform={`rotate(${index * 90 + shown.draft.rotationDeg} 16 16)`}><path d="M9 24V13Q9 9 13 9H24" fill="none" stroke="rgba(36,45,51,.24)" strokeWidth="9" strokeLinecap="round"/><path d="M9 23V13Q9 9 13 9H23" fill="none" stroke="rgba(255,255,255,.85)" strokeWidth="6" strokeLinecap="round"/></g></svg></button>
  return <div data-sticker-editor={shown.source.stickerId} data-editor-phase={shown.phase} data-hud-presence={presence.toFixed(3)} data-hud-release={release?.progress ?? 1} inert={state === null} role="group" aria-label="Sticker controls" className="pointer-events-none fixed inset-0 z-40" style={{ opacity: Math.min(1, presence) }} onKeyDown={event => { event.stopPropagation(); if (event.key === 'Escape') { event.preventDefault(); if (deviceStore.get(stickerEditorGestureAtom)) cancel(); else if (dismissStickerTooltip()) { /* Keep the selected sticker. */ } else { const id = shown.source.stickerId; dismissStickerEditor(); document.querySelector<HTMLElement>(`[data-sticker-placed="${id}"]`)?.focus() } } else if (event.key === '[' || event.key === ']') { event.preventDefault();deviceStore.set(tooltipAtom,null); setStickerEditorProperty('width'); previewStickerEdit(shown.draft.width + (event.key === '[' ? -.005 : .005)); void applyStickerEditor(place) } else if ((event.metaKey || event.ctrlKey) && event.key === 'z') { event.preventDefault(); void undoStickerEdit(place) } }}>
    <style>{`
      ...
      @media (prefers-reduced-transparency:reduce) { .contour-glass { background:#f3f5f6; backdrop-filter:none; } .contour-grip path:last-child { stroke:#fff; } }
      @media (prefers-contrast:more) { .contour-glass { background:#fff; border-color:#263139; } [data-hud-wear]:focus-within { outline:2px solid white; outline-offset:2px; border-radius:22px; }
      .contour-grip { filter:drop-shadow(0 0 1px #000); } }
      @media (prefers-reduced-motion:reduce) { .contour-grip:active svg { animation:none; } }
    `}</style>
    <svg data-sticker-contour className="pointer-events-none absolute inset-0 h-full w-full" aria-hidden="true"><g fill="none" strokeLinejoin="round">{shape.paths.map((path, i) => <path key={i} d={`M${path.map(p => `${p.x},${p.y}`).join('L')}Z`} stroke="rgba(255,255,255,.85)" strokeWidth="1"/>)}</g></svg>
    {grips.map(handle)}
    ...
    {tooltipView}
    {shown.message !== null && <span role="status" className="fixed max-w-56 rounded bg-[#242a2e] px-2 py-1 text-[11px] text-white" style={{ left: arrangement.tools.x, top: arrangement.tools.y + 52 }}>{shown.message}</span>}
