...
interface Entry {
  ...
  readonly reservation: number; producer: Worker | null; refs: number; result: StickerTransactionResult | null; bytes: number; touched: number;
}
...
function entryCost(entry: Entry): number { return entry===active&&!entry.result ? entry.reservation : inputBytes(entry.input)+entry.bytes*(entry.producer===worker&&worker!==null?2:1); }
function accountedBytes(): number { return privateBytes+[...entries.values()].reduce((sum,entry)=>sum+entryCost(entry),0); }
...
function makeRoom(additional: number) {
 for(const entry of [...entries.values()].filter(entry=>entry.refs===0&&entry.result!==null).sort((a,b)=>a.touched-b.touched)){if(accountedBytes()+additional<=MAX_RETAINED_BYTES)break;removeEntry(entry);}
}
...
function evict() {
  ...
  let idleBytes = 0;
  for (const entry of entries.values()) if (entry.refs === 0) idleBytes += entryCost(entry);
  const idle = [...entries.values()].filter(entry => entry.refs === 0 && entry.result !== null).sort((a, b) => a.touched - b.touched);
  for (const entry of idle) if (now - entry.touched >= IDLE_MS || idleBytes > MAX_IDLE_BYTES || entries.size >= MAX_JOBS) { idleBytes -= entryCost(entry); removeEntry(entry); }
...
function complete(entry: Entry, result?: StickerTransactionResult, error?: Error) {
  ...
  clearTimeout(deadline); deadline = undefined; fallback = null; active = null;
...
function recover(entry: Entry) {
  ...
  clearTimeout(deadline); deadline = undefined; stopWorker(); workerFailed = true; entry.producer = null;
...
function pump() {
  ...
  makeRoom(entry.reservation - inputBytes(entry.input));
  if (accountedBytes() + entry.reservation - inputBytes(entry.input) > MAX_RETAINED_BYTES) { entries.delete(entry.key); entry.reject(new Error('Sticker computation byte capacity exceeded')); pump(); return; }
  active = entry;
  ...
  try {
    ...
    if (input.kind === 'contour') {
      const damage = entries.get(input.damageKey), geometry = input.geometryKey ? entries.get(input.geometryKey) : undefined;
...
export function acquireStickerTransaction(key: string, input: StickerTransactionRequest): StickerTransactionLease {
  ...
  if (!entry) {
    ...
    entry = { key, input, id: ++sequence, promise, resolve, reject, reservation, producer: null, refs: 0, result: null, bytes: 0, touched: performance.now() };
...
export async function acquirePrivateStickerTransaction(key: string, input: StickerTransactionRequest, port: MessagePort, signal: AbortSignal): Promise<{ readonly resourceId: number; release(): void }> {
  const main = acquireStickerTransaction(key, input);
  let reserved = 0, released = false, admitted = false;
  const release = () => { if (released) return; released = true; privateBytes -= reserved; if (admitted) privateOwners--; main.release(); };
  try {
    signal.throwIfAborted();
    if (input.kind === 'fit') throw new Error('A fit result is not a renderer resource');
    const result = await new Promise<StickerTransactionResult>((resolve, reject) => {
      const abort = () => { main.release(); reject(abortError()); };
      signal.addEventListener('abort', abort, { once: true });
      void main.result.then(value => { signal.removeEventListener('abort', abort); resolve(value); }, error => { signal.removeEventListener('abort', abort); reject(error); });
    });
    signal.throwIfAborted();
    ...
    reserved = stickerTransactionBuffers(result).reduce((sum, buffer) => sum + buffer.byteLength, 0) * 2;
    makeRoom(reserved);
    if (privateOwners >= MAX_JOBS || accountedBytes() + reserved > MAX_RETAINED_BYTES) { reserved = 0; throw new Error('Private sticker byte capacity exceeded'); }
    privateBytes += reserved; privateOwners++; admitted = true;
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      signal.addEventListener('abort', abort, { once: true });
      try { producer.postMessage({ type: 'deliver', id: entry.id, deliveryId: resourceId, port }, [port]); }
