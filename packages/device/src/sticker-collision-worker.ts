...
self.onmessage = (event: MessageEvent<CollisionWorkerFace[]>) => {
  const faces = event.data.map(face => {
  ...
  try {
    ...
    self.postMessage({ snapshot }, { transfer: [snapshot.coordinates.buffer, snapshot.provenance.buffer] });
    collider.dispose();
  } catch (error) {
    self.postMessage({ error: error instanceof Error ? error.message : 'Collision preparation failed' });
