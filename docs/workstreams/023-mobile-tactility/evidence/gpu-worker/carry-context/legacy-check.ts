...
const until = async (check: () => boolean) => {for (let index = 0; index < 1000; index++) {if (check()) return; await new Promise(resolve => setTimeout(resolve, 5));} throw Error('Deadline '+JSON.stringify({error:runtime.getSnapshot().error,computed,workers:workers.length,terminated:workers.map(w=>w.terminated)}));};
...
try {
  ...
  await Bun.write(new URL('./legacy-check.json', import.meta.url), JSON.stringify({actualModuleWorker: true, mainPrivateExact: true, boundsWearStampParity: true, mountedBuffersNeverDetached: true,
