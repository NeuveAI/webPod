...
export interface CarryWorkerResult { readonly id: number; readonly geometry?: PaperGeometryTransfer; readonly wearGeometry?: PaperGeometryTransfer | null; readonly pointerError?: number | null; readonly error?: string }
...
self.onmessage = ({ data }: MessageEvent<CarryWorkerMessage>) => {
  if ('context' in data) {
    rear?.dispose(); collider?.dispose(); source?.dispose(); target?.dispose(); source = null; target = null; sourceKey = ''; targetKey = '';
  ...
  try {
    ...
    const wearGeometry = wear ? transferPaperGeometry(wear) : null;
    ...
    self.postMessage({ id: data.id, geometry, wearGeometry, pointerError: result.pointerError } satisfies CarryWorkerResult, { transfer });
