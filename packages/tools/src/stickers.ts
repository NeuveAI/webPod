...
/** The nine sticker tools use the current draft's ordinary JSON results and cancellation. */
export function createStickerTools(controls: () => StickerToolControls, mutate: InteractionMutation = createInteractionMutation()): readonly NativeTool[] {
  ...
  return [
    ...
    empty('webpod_release_sticker', 'Release the held sticker to its exact original saved placement or collection. Discards draft rotation, wear and position. Does not write inventory; cannot undo a placement already accepted by persistence.', 'release'),
