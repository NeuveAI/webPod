...
interface Job {signal: AbortSignal; finish(): void; reject(): void; released: number; queryReleased: number; transferred: number}
const jobs: Job[] = [];
mock.module('../../../../../../packages/composite/src/native-pack-resources', () => ({
  prepareNativePackFrame(recipe: StickerPackNode, _scene: DeviceStickerScene, signal: AbortSignal, options?: {readonly prepareContours?: boolean}): Promise<Owner> {
    assert.equal(options?.prepareContours, false);
    return new Promise((resolve, reject) => {
      const job: Job = {signal, released: 0, queryReleased: 0, transferred: 0, reject() {reject(new Error('producer failure'));}, finish() {
        ...
        resolve({frame: {key: 'controlled', recipe, prints, geometry: [], damage: [], artworks: []}, prepared: new Map(), queryPrints: new Map(), releaseQueries() {if (!job.queryReleased) job.queryReleased++;}, release() {job.released++; if (!job.queryReleased) job.queryReleased++;}, transferred() {job.transferred++;}});
      }};
      jobs.push(job);
    });
  },
...
const fourth = fixture(); fourth.owner.update(fourth.scene); const held = job(); held.finish(); await tick(); fourth.owner.dispose(); fourth.owner.ack(current(fourth)); assert.equal(held.released, 0); assert.equal(held.queryReleased, 0); fourth.owner.rendererRetired(); fourth.owner.rendererRetired(); assert.equal(held.released, 1); assert(!fourth.events.includes('ready:a')); checks++;
