...
for(const colourway of ['black','white']){
 ...
 for(const [name,pitchDeg,yawDeg] of [['front',0,0],['quarter',20,-55],['rear',0,180],['rear-quarter',10,146]] as const){
  ...
  if(name==='front'){const clip=await panel.boundingBox();if(clip!==null)await page.screenshot({path:resolve(folder,`${colourway}-lcd.png`),clip});}
 }
}
