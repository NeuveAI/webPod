...
## Committed archive integration regression

Reviewer found git-archive runner omitted new Vite sticker asset hook and source assets. Narrow fix in scripts/browser-source-fingerprint.ts now archives present sticker script/PLAYWORN folder and fingerprints script, manifest and all60 PNG bytes. Extended existing tests execute the archived copy hook in Bun, assert60 validated generated assets, output fingerprint stability, excluded roots and independent mutation sensitivity.5tests37assertions+scoped lint pass. Historical8507a63 test is explicitly extraction-only: replay browser suites with matching archived config/helper; no mixed fingerprint algorithm redesign. Full evidence in evidence/regression/archive-fix.md.

Coordination correction: live helper edits affected reviewer later worker expectations during a browser run even though served snapshot was immutable. Reviewer halted invalid remainder; source/helper/tests now frozen before review/regression restart. Only excluded docs changed after freeze. No new source commits until independent reviewer approval.

Archive follow-up approved independently. All13 existing device browser regressions pass on frozen helper342-file fingerprint; full evidence and scoped review amendment retained. Lead scripts TypeScript check passed (attributed). Only narrow helper/test plus reviewed evidence and checkpoint documentation staged for the follow-up; no new source edits after review, no authentication completeness claim.
