...
Prepared before implementation; identity strategy pending owner answer. All mutable operations derive owner from server session, never request userId.
...
- `POST /api/stickers/session`: dedicated private MusicKit credential transfer; verify upstream before creating active session/import. No token in shared Session or Jotai. Identity continuation behavior pending owner answer.
...
- `DELETE /api/stickers/session`: cancel active session access; recovery behavior pending identity decision.
