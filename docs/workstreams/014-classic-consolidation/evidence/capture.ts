...
for (const colourway of ['black', 'white'] as const) {
 for (const [view, yawDeg] of [['front', 0], ['oblique', 40], ['side', 85], ['reflection', 18], ['bottom', 25]] as const) {
  await page.evaluate(({ colourway, yawDeg, pitchDeg }) => { window.__webpodDevicePreview?.setColourway(colourway); window.__webpodDevicePreview?.setOrientation({ pitchDeg, yawDeg, rollDeg: 0 }); }, { colourway, yawDeg, pitchDeg: view === 'bottom' ? -55 : view === 'reflection' ? -20 : 0 });
