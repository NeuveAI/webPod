...
try{
 ...
 const placement={stickerId:a.id,surface:'back' as const,x:.5,y:.5,width:.25,rotationDeg:0,wear:0};
 ...
 const input={kind:'surface' as const,art:a,placement,form:DEFAULT_DEVICE_FORM};let current:PreparedPrint|null=null;let previousGeometry:BufferGeometry|null=null;
 for(let i=0;i<4;i++){
  phase=`restored-surface:${i}`;const surface=await prepareSurface({input,custom},new AbortController().signal);geometryOwners.push(surface.release);
  if(previousGeometry){assert.notEqual(surface.geometry,previousGeometry);for(const name of ['position','normal','uv'])assert.equal(surface.geometry.getAttribute(name).array,previousGeometry.getAttribute(name).array);}
  const options=process.env.GL_CURRENT_REUSE==='1'&&current?{reuseDamage:preparedStickerResources(current.geometry)}:undefined;
  const next=await preparePrint({id:a.id,texture:await textureFor(a.id),geometry:surface.geometry,wearGeometry:surface.geometry,wear:0},new AbortController().signal,options);owners.push(next);stages.push({phase,transactions:inspectStickerTransactions()});current=next;previousGeometry=surface.geometry;
 }
