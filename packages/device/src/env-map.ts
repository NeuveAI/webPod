...
export const DEFAULT_ENV_ROOM: EnvRoomParams = {
  ...
  elevHalfSpanDeg: 14.8375,
  stops: STEEL_STOPS,
  radianceGain: radianceGainForMirror(DEFAULT_DEVICE_MATERIALS.steelBack.color),
  exposure: 1.0188,
  profileContrast: 1.1081,
  profileSharpenAmount: 0.42,
  profileSharpenSigma: 0.034,
  profileSharpenAmount2: 0,
  profileSharpenSigma2: 0.01,
  sky: {
...
function buildProfile(params: EnvRoomParams): Float32Array {
  ...
  for (let i = 0; i < PROFILE_SAMPLES; i++) {
    ...
    for (let c = 0; c < 3; c++) {
      ...
      raw[i * 3 + c] = Math.min(1, Math.max(0, pivot + (value - pivot) * contrast))
    }
  }

  const once = unsharp(raw, params.profileSharpenAmount, params.profileSharpenSigma)
  return unsharp(once, params.profileSharpenAmount2, params.profileSharpenSigma2)
}

/** One unsharp pass over the interleaved RGB profile: `orig + amount·(orig − blur)`. */
function unsharp(profile: Float32Array, amount: number, sigmaFraction: number): Float32Array {
  ...
  for (let i = 0; i < PROFILE_SAMPLES; i++) {
    for (let c = 0; c < 3; c++) {
      ...
      out[i * 3 + c] = Math.min(1, Math.max(0, original + amount * (original - blurred)))
    }
  }
  return out
}
...
function readProfile(profile: Float32Array, t: number): [number, number, number] {
...
function sampleStops(stops: ReadonlyArray<RoomStop>, t: number): [number, number, number] {
  const clamped = Math.min(1, Math.max(0, t))
  const first = stops[0]
 ...
 */
export function createRoomEnvMap(params: EnvRoomParams = DEFAULT_ENV_ROOM): DataTexture {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      const dir: [number, number, number] = [rho * Math.cos(a), dirY, rho * Math.sin(a)]

      // Elevation against the *tilted* axis.
      const e = Math.min(1, Math.max(-1, dir[0] * axis[0] + dir[1] * axis[1] + dir[2] * axis[2]))
      ...
      const facing = dir[0] * skyDir[0] + dir[1] * skyDir[1] + dir[2] * skyDir[2]
