...
function cssNumber(prop: string): number {
  const match = new RegExp(`--${prop}:\\s*([0-9.]+)\\s*(?:px|deg)?\\s*;`).exec(
    CSS,
  );
  if (match?.[1] === undefined)
    throw new Error(`--${prop} not found in globals.css`);
...
const CSS_ONLY: ReadonlyArray<readonly [string, string]> = [
  [
    "fx-peak-rotate",
    "peak alpha for the rotate event; consumed only by the CSS keyframes",
  ],
  [
    "fx-peak-press",
    "peak alpha for the press event; consumed only by the CSS keyframes",
  ],
];
...
describe("globals.css and the typed exports agree", () => {
  ...
  test("every numeric --fx-* / --panel-* token is either bound or declared CSS-only", () => {
    ...
    const declared = [
      ...CSS.matchAll(/--((?:fx|panel)-[a-z0-9-]+):\s*[0-9.]/g),
...
describe("D-020 is applied to all three @theme blocks", () => {
  test("every @theme block is static, so nothing is tree-shaken out of the build", () => {
    const blocks = [...CSS.matchAll(/^@theme\b[^{]*\{/gm)].map((m) =>
      m[0].trim(),
    );
