...
try{
 ...
 try{
  ...
  for(let epoch=1;epoch<=4;epoch++){
   phase=`mounted-restored-surface:${epoch}`;const surface=await prepareSurface({input,custom},new AbortController().signal);geometryOwners.push(surface.release);
   ...
   if(previous){assert.equal(read().damage.field,previous.damage.field);assert.equal(read().geometry.getAttribute('normal').array,previous.geometry.getAttribute('normal').array);}
