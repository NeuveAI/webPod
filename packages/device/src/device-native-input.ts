...
export function createDeviceNativeInput(input: {
  ...
  // Only a successful adapter admission owns this suppression. The app keeps
  // capture, final samples and velocity; unrelated captures never imply a grab.
  let orientationPointer: number | null = null, orientationGeneration = 0;
  const clearOrientation = () => {
    orientationGeneration++;
    if (orientationPointer === null) return;
    orientationPointer = null;
    setDeviceControlCursor(canvas, false);
    input.onOrientationGrabHoverChange?.(false);
  };
  ...
  const down = (event: PointerEvent) => {
    ...
    if (start) {
      ...
      if (input.onOrientationGrabStart?.(start)) {
        // Admission can synchronously dispose, blur or release the owner.
        if (!disposed && generation === orientationGeneration && canvas.hasPointerCapture(event.pointerId)) {
          orientationPointer = event.pointerId;
          setDeviceControlCursor(canvas, false);
          input.onOrientationGrabHoverChange?.(false);
        }
  ...
  const move = (event: PointerEvent) => {
   ...
   if (orientationPointer === event.pointerId) {
  ...
  const up = (event: PointerEvent) => { orientationGeneration++; if (orientationPointer === event.pointerId) clearOrientation(); syncFront(); const channel = active.get(event.pointerId); active.delete(event.pointerId); if (channel) events[channel].up(sample(event)); };
  const cancel = (event: PointerEvent) => { orientationGeneration++; if (orientationPointer === event.pointerId) clearOrientation(); active.delete(event.pointerId); };
  ...
  window.addEventListener('blur', clearOrientation);
  canvas.addEventListener('pointercancel', cancel); canvas.addEventListener('lostpointercapture', cancel);
  const dispose = () => {
    if (disposed) return; disposed = true; clearOrientation();
    window.removeEventListener('blur', clearOrientation);
