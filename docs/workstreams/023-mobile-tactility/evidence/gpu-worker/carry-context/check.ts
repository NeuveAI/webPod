...
const workers: ObservedWorker[] = []; let live = 0, peak = 0, installedBytes = 0, peakInstalledBytes = 0, failContext = false;
...
try {
  runtime.request(input, shells.back, visibility); assert.equal(workers.length, 0);
  ...
  assert.equal(workers.length, 0); await until(() => !!runtime.getSnapshot().frame);
  ...
  assert.equal(workers.length, 1); assert.equal(peak, 1);
  ...
  doc.hidden = false; doc.dispatchEvent(new Event('visibilitychange')); assert.equal(workers.length, 1);
  ...
  pending.request(input, shells.back, visibility); stopPending(); await sleep(30); assert.equal(workers.length, before);
  assert.equal(pending.inspectRendererResources().bytes, 0); output.disposePendingNoLateWorker = true;
  failContext = true;
  const failed = createCarryPreparation({ renderer: true }), stopFailed = failed.mount(); failed.request(input, shells.back, visibility);
