...
| Change the native action target from 44px to the reviewer's exact 20px plant | `bun test packages/panel/src/Panel.test.tsx` and package Playwright filtered to `axe and preference` | both exit 1; source gate rejects 20px and browser reports native `offsetWidth: 20` below 44 |

The compliant 44px target plus negative margins produces a provider screenshot byte-identical to the prior 24px visual composition (`cmp` exit 0).
