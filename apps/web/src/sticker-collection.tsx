...
const PACK = { ...STICKER_PACK_LAYOUT, peelTravelPx: 64, placementWidth: 0.25, releaseInertiaSeconds: 0.025, titleHandoff: .2, chromeTravelPx: 8 } as const
...
interface RearCandidate { readonly event: globalThis.PointerEvent; readonly canvas: HTMLCanvasElement; readonly source: StickerPlacement; readonly grab: StickerGrab | null }
...
function clearRearCandidate(): RearCandidate | null {
  return releaseStickerCandidate(() => deviceStore.get(rearCandidateAtom), () => deviceStore.set(rearCandidateAtom, null))
}
...
export function StickerCollection({ orientation, commands }: { readonly orientation: DeviceOrientation; readonly commands: StickerCollectionCommands }) {
  ...
  const poseKey = useRef(JSON.stringify(orientation))
  const toolActions = useRef<StickerUiActions | null>(null)
  ...
  const move = (event: Pick<globalThis.PointerEvent, 'pointerId' | 'clientX' | 'clientY' | 'timeStamp' | 'stopPropagation'>): void => {
    ...
    const moveGeneration = getStickerInteractionGeneration(), moveOwner = deviceStore.get(stickerSourceAnchorAtom)
    const ownsMove = () => moveGeneration === getStickerInteractionGeneration() && deviceStore.get(pointerAtom)?.pointerId === pointer.pointerId && deviceStore.get(stickerSourceAnchorAtom) === moveOwner
    if (pointer.grab?.isValid?.() === false) { releaseCapturedStickerPointer(captureTarget.current); cancelStickerInteraction(); return }
    ...
    if (pointer.kind === 'pull') updateStickerInteraction({ progress: pointer.startProgress + delta / pointer.travel })
      ...
      if (!publishOwnedStickerPull(ownsMove, () => {
        if (source != null && moveOwner !== null) updateStickerSourcePull(source, { x: event.clientX - pointer.startX, y: event.clientY - pointer.startY }, moveOwner)
      })) return
  ...
  const keyboardPlace = (id = selected?.id): void => {
    ...
    run(async (isCurrent) => { await commands.place(placement); if (!isCurrent()) return
      animateStickerValue('landing', { position: deviceStore.get(stickerInteractionAtom).landing, velocity: 0, target: 1 }, reducedMotion, () => {
        animateStickerValue('peel', { position: deviceStore.get(stickerInteractionAtom).peel, velocity: 0, target: 0 }, reducedMotion, () => { cancelStickerInteraction(); deviceStore.set(stickerDetailIdAtom, null); deviceStore.set(collectionMessageAtom, `${selected.name} stuck to your iPod.`); document.querySelector<HTMLElement>(`[data-sticker-placed="${placement.stickerId}"]`)?.focus() })
      })
    })
  }
  const switchCollection = (direction: number): void => {
    ...
    deviceStore.set(stickerDetailIdAtom, null)
    resetStickerCarry()
    updateStickerInteraction({ stage: 'open' })
    // STORYBOARD: tuck the liner → turn edge-on → exchange ready artwork → unfold.
    animateStickerValue('sheet', { position: deviceStore.get(stickerSheetRevealAtom), velocity: -1, target: 0 }, reducedMotion, () => {
      animateStickerValue('turn', { position: 0, velocity: 1, target: 1 }, reducedMotion, () => {
        deviceStore.set(selectedStickerGenreAtom, next.genre)
        const first = next.slots[0]
        if (first === undefined) return
        void waitForStickerSheet(first.art.id, isCurrent).then(() => {
          if (!isCurrent()) return
  ...
  useEffect(() => {
    const clearCandidate = clearRearCandidate
    const down = (event: globalThis.PointerEvent): void => {
      if (compositeTier.tier !== 'T1' || !event.isPrimary || event.button !== 0 || !(event.target instanceof Element) || event.target.closest('[data-sticker-editor],[data-sticker-editor-failure],[data-sticker-collection],button,a,input') !== null) return
      ...
      const picked = pickStickerSource(rear, commands, event.clientX, event.clientY)
      if (picked === null) { dismissStickerEditor(); return }
      const { source, grab } = picked
      event.preventDefault(); event.stopImmediatePropagation()
      clearCandidate()
      canvas.setPointerCapture(event.pointerId)
      deviceStore.set(rearCandidateAtom, { event, canvas, source, grab })
    }
    const moved = (event: globalThis.PointerEvent): void => {
      ...
      if (candidate !== null && candidate.event.pointerId === event.pointerId) {
        if (candidate.grab?.isValid?.() === false) { clearCandidate(); return }
        ...
        dismissStickerEditor()
        deviceStore.set(selectedStickerGenreAtom, own.genre)
        start(candidate.event, candidate.canvas, 'rear', slot, candidate.source, candidate.grab)
        move(event)
    ...
    const escaped = (event: globalThis.KeyboardEvent): void => {
      ...
      if (deviceStore.get(rearCandidateAtom) !== null) { event.preventDefault(); event.stopImmediatePropagation(); clearCandidate(); return }
  ...
  // The tool registry points at the same current handlers as the mounted overlay.
  useEffect(() => {
    toolActions.current = {
  ...
  useEffect(() => mountStickerToolControls(() => {
    const actions = toolActions.current
    if (actions === null) throw new Error('Sticker UI handlers are not ready.')
    return actions
  }), [])
  ...
  return (
    <div ref={host} inert={!rear} aria-hidden={!rearUiVisible} style={{ opacity: rearUiVisible ? 1 : 0, transform: `translateY(${rearUiVisible || reducedMotion ? 0 : REAR_UI_MOTION.travelPx}px)`, transition: `opacity ${presenceMs}ms ${presenceEase}, transform ${presenceMs}ms ${presenceEase}` }} className="pointer-events-none absolute inset-0 z-20" data-sticker-turn={packTurn} data-sticker-stage={interaction.stage} data-sticker-reduced-motion={reducedMotion} data-sticker-progress={interaction.progress} data-sticker-workspace-lowering={workspaceLowering} data-sticker-sheet-reveal={sheetReveal} data-sticker-peel={interaction.peel} data-sticker-landing={interaction.landing} onKeyDown={(event) => { event.stopPropagation(); if (event.key === 'Escape') { event.preventDefault(); if (deviceStore.get(stickerInteractionAtom).sourcePlacement != null) { releaseCapturedStickerPointer(captureTarget.current); returnStickerToSheet(reducedMotion) } else if (detail !== undefined) deviceStore.set(stickerDetailIdAtom, null); else close() } }}>
      ...
      {(rear || packPresence > .001) && usable && editor === null ? <div className="pointer-events-auto absolute text-[#29291f]" data-sticker-collection={collection?.genre} style={{ left: layout.centerX - layout.width / 2, width: layout.width, height: layout.height, bottom: PACK.bottomGapPx, transformOrigin: `50% ${layout.height / 2 + layout.height * PACK.linerTravel * sheetReveal}px`, transform: `translateY(${(1 - packPresence) * (layout.height * (interaction.progress + PACK.linerTravel * sheetReveal) + PACK.teasePx + PACK.bottomGapPx) + (layout.height + PACK.bottomGapPx - PACK.teasePx) * (1 - interaction.progress) - layout.height * PACK.linerTravel * sheetReveal + (viewport.width < PACK.desktopBreakpoint ? layout.height * PACK.linerTravel * workspaceLowering : 0)}px) perspective(1000px) rotateY(${presentation.turnRadians}rad)` }}>
        ...
        {interaction.progress > .01 ? <>
          {collection !== null ? <>
            <div aria-hidden="true" className="pointer-events-none absolute inset-x-5 top-[52px]" style={{ opacity: presentation.chrome * presentation.textOpacity * (1 - Math.min(1, sheetReveal / PACK.titleHandoff)), transform: `translateY(${layout.height * PACK.linerTravel * sheetReveal}px)` }}>
              <p className="text-[22px] font-black leading-none tracking-[-.04em]" style={{ fontFamily: 'Impact, Haettenschweiler, Arial Narrow, sans-serif' }}>{collection.title}</p>
            ...
            <div className="pointer-events-none absolute inset-x-5 top-[52px]" style={{ opacity: presentation.chrome * presentation.textOpacity * Math.min(1, sheetReveal / PACK.titleHandoff), transform: `translateY(${(1 - presentation.chrome) * PACK.chromeTravelPx}px)`, clipPath: `inset(0 0 max(0px, calc(100% - ${layout.height * PACK.linerTravel * sheetReveal - 52}px)) 0)` }}>
            ...
            {sheetReveal > .02 ? collection.slots.map((slot, index) => {
