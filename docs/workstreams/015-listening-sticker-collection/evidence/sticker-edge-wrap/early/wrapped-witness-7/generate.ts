import {intersectStickerPrint} from '/Users/vinicius/code/webPod/packages/device/src/sticker-hit';
...
import { Matrix4, PerspectiveCamera, Object3D, Vector2, Vector3, Mesh, MeshBasicMaterial, MeshPhysicalMaterial, Texture, FrontSide, DoubleSide, Raycaster } from '/Users/vinicius/code/webPod/packages/device/node_modules/three/build/three.module.js';
...
// Exercise actual production alpha hit→capture handoff with decoded source bytes.
class SourceImage {complete=true;naturalWidth=mask.width;naturalHeight=mask.height;}
const imageDescriptor=Object.getOwnPropertyDescriptor(globalThis,'HTMLImageElement'),documentDescriptor=Object.getOwnPropertyDescriptor(globalThis,'document'),rgba=new Uint8ClampedArray(mask.pixels.length*4);for(let i=0;i<mask.pixels.length;i++)rgba[i*4+3]=mask.pixels[i];
Object.defineProperty(globalThis,'HTMLImageElement',{configurable:true,value:SourceImage});Object.defineProperty(globalThis,'document',{configurable:true,value:{createElement:()=>({getContext:()=>({drawImage(){},getImageData:()=>({data:rgba})})})}});
const sourceTexture=new Texture(new SourceImage());
...
const results=[]; const failures=[];for(const viewport of[1280,375])for(const face of['side','front']){
 ...
 const mesh=new Mesh(g,new MeshPhysicalMaterial({side:FrontSide,map:sourceTexture}));mesh.updateMatrixWorld();const ray=new Raycaster();const visible=[],hidden=[];
 ...
 const actualHit=intersectStickerPrint(worldRay,mesh,placement);if(actualHit?.object!==mesh)throw Error('Production hit did not retain rendered mesh');if(!actualHit?.uv||!actualHit.face)throw Error('Original pixel no longer hits painted ink');
...
await Bun.write(out+'candidates.json',JSON.stringify({scope:'Actual captured equipped buffers/matrices. Shell witness uses exact source rear/front/glass/wheel/select fixture; missing hardware is not claimed full assembly.',bufferHashes,results,failures},null,2));collider.dispose();g.dispose();shell.dispose();sourceTexture.dispose();if(imageDescriptor)Object.defineProperty(globalThis,'HTMLImageElement',imageDescriptor);else Reflect.deleteProperty(globalThis,'HTMLImageElement');if(documentDescriptor)Object.defineProperty(globalThis,'document',documentDescriptor);else Reflect.deleteProperty(globalThis,'document');
