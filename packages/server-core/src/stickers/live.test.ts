...
describe('device collection sessions', () => {
  ...
  test('failed library import returns explicit failed inventory without erasing prior grants', async () => {
    ...
    await a.request('/packs/open', 'POST', { packId: before.packs[0].id })
    const placed = await (await a.request('/placements', 'PUT', { revision: 0, placements: [{ stickerId: before.stickerIds[0], surface: 'back', x: 0.5, y: 0.5, width: 0.2, rotationDeg: 0 }] })).json()
    fail = true; f.advance()
    ...
    expect((await a.request('/session', 'POST', { musicUserToken: 'synthetic' })).status).toBe(429)
    fail = false; f.advance()
    const recovered = await (await a.request('/session', 'POST', { musicUserToken: 'synthetic' })).json()
    expect(recovered.importStatus).toBe('complete'); expect(recovered.packs).toEqual(placed.packs); expect(recovered.placements).toEqual(placed.placements)
  })
