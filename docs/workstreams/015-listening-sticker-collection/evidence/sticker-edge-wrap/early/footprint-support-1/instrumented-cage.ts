...
import { frontShellOffsetAt } from '/Users/vinicius/code/webPod/packages/device/src/front-surface';
import { stickerInsertBridgeHeight } from '/Users/vinicius/code/webPod/packages/device/src/sticker-insert-bridge';
import type { createStickerStripProfile } from '/Users/vinicius/code/webPod/packages/device/src/sticker-strip';
...
export function createStickerCornerCage(
  ...
  const diagnostics={calls:0,minDepthResidual:Infinity,negativeDepthResiduals:0,maxEquationResidual:0,worstEquation:null as unknown,minReachedDot:Infinity,worstDirection:null as unknown};
  ...
  const recordDirection=(p:{nx:number;ny:number},depth:number,arc:number)=>{const dot=p.nx*nx+p.ny*ny;if(dot<diagnostics.minReachedDot){diagnostics.minReachedDot=dot;diagnostics.worstDirection={depth,arc,N:[nx,ny],normal:[p.nx,p.ny]};}};
  const mapped = (depth: number, arc: number) => {
    ...
    if(depth>0)recordDirection(p,depth,arc);
  ...
  const rawPosition = (dx: number, dy: number) => {
    ...
    diagnostics.calls++;diagnostics.minDepthResidual=Math.min(diagnostics.minDepthResidual,nonlinearDepth);if(nonlinearDepth< -1e-7)diagnostics.negativeDepthResiduals++;
  ...
  return { diagnostics, point: position, sample(dx: number, dy: number, lift = .18) {
