...
const path='/private/var/folders/ft/7tsjkpcn20q5fx1q8dwv26x80000gn/T/webpod-native-rotation-interval.json.gz';
const raw=await Bun.file(path).arrayBuffer();const events:Event[]=JSON.parse(gunzipSync(raw).toString()).traceEvents;
const min=260412205264,max=260420943964,pid=39318,main=4511411,renderer=4534077;
...
const lifecycle = {targetFrameProcessSwapEvents: target.filter(e => ['FrameCommittedInBrowser', 'FrameDeletedInBrowser', 'CommitLoad', 'ProcessReadyInBrowser'].includes(e.name)).length, explicitPauseHiddenBfcacheEvents: target.filter(e => /DebuggerPaused|PageHidden|BFCache/.test(e.name)).length,  note: 'No explicit matching lifecycle marker is proof of neither absence nor cause; incomplete main RunTask cannot be assigned a completed duration.'};
const coverage={mainIdentity:"Prior same-process entrance main tid; current capture has no CrRendererMain metadata or main profile",targetThreadNames:target.filter(e=>e.name==="thread_name").map(e=>({tid:e.tid,name:e.args?.name})),inputEvents:target.filter(e=>/EventDispatch|EventTiming|InputLatency|Pointer|Touch/.test(e.name)).length,unclosedProfilerStart:target.filter(e=>e.name==="CpuProfiler::StartProfiling").map(e=>({tid:e.tid,ts:e.ts,phase:e.ph})),gestureConfirmed:false};
const output={coverage,lifecycle,sourceCommit:'2856a5a plus frozen warmup source hashes',traceSha256:createHash('sha256').update(new Uint8Array(raw)).digest('hex'),traceBytes:raw.byteLength,interval:{min,max,durationMs:(max-min)/1000},target:{pid,main,renderer},tracks,profiles,
 limitations:['No target main-thread events or CPU profile in the authoritative interval; absence is not zero CPU cost.','Profile time-delta attribution is sampling weight, not exact function duration or GPU time.','No confirmed drag or target input events; cannot establish rotation workload, motion cadence or responsiveness.','Only sampled symbol is Three Animation.start update; first delta2218.399ms overlaps profiler startup and is not trustworthy function CPU cost.99remaining sample deltas total513.459ms, also insufficient for rotation attribution.','Global trace contains other processes; analysis intentionally excludes them.']};
