...
const folder=resolve(import.meta.dirname,'final-studio');await mkdir(folder,{recursive:true});
...
for(const lighting of ['combined','key-only','fill-only','rim-only','environment-only']){
 await page.goto(`http://localhost:3000/_spike/device?capture=&diagnostic=production-surface&colourway=black&lighting=${lighting}`);await page.waitForFunction(()=>Reflect.get(window,'__webpodDevicePreview')!==undefined);await pose(20,-55);await page.screenshot({path:resolve(folder,`black-quarter-${lighting}.png`)});
}
const sweep=[];
for(const colourway of ['black','white']){
 ...
 for(const pitch of [-30,0,30])for(const yaw of [140,160,180,-160,-140]){
