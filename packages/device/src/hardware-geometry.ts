...
export function createHardwareGeometry(form: DeviceFormParams): readonly HardwarePart[] { return drainSteps(createHardwareGeometrySteps(form)); }
export function* createHardwareGeometrySteps(form: DeviceFormParams): Generator<void, readonly HardwarePart[], void> {
  const parts: HardwarePart[] = [];
  const add = function* (name: string, shape: Shape, x: number, z: number, side: 1 | -1,
  ...
  yield* add("device-hold-collar", roundFrame(hold.width, hold.depth, hold.radius,
    hold.width - 2, hold.depth - 2, hold.radius - 1), hold.x, hold.z, 1, -1.5, 0.2, "metal");
  yield* add("device-hold-recess", roundedRectShape(hold.width - 1, hold.depth - 1, hold.radius - 0.5),
    hold.x, hold.z, 1, -hold.cavityDepth, -hold.cavityDepth + 0.5, "cavity");
  yield* add("device-hold-slot-wall", roundFrame(hold.width - 1, hold.depth - 1, hold.radius - 0.5,
    hold.width - 3, hold.depth - 3, hold.radius - 1.5), hold.x, hold.z, 1, -hold.cavityDepth, -0.5, "insulator");
  yield* add("device-hold-indicator", roundedRectShape(hold.width - 3, hold.depth - 3, hold.radius - 1.5),
    hold.x, hold.z, 1, -1.5, -1.1, "orange");
  yield* add("device-hold-slider", roundedRectShape(hold.sliderWidth, hold.sliderDepth, hold.sliderDepth / 2, 16),
  ...
  yield* add("device-headphone-rim", collar, jack.x, jack.z, 1, -0.65, 0.35, "metal");
  const barrel = disc(jack.boreRadius + 0.4);
  barrel.holes.push(circleHole(0, 0, jack.boreRadius - 0.65, 64));
  yield* add("device-headphone-barrel", barrel, jack.x, jack.z, 1, -jack.cavityDepth, -0.55, "insulator");
  yield* add("device-headphone-well", disc(jack.boreRadius), jack.x, jack.z, 1, -jack.cavityDepth - 0.4, -jack.cavityDepth, "cavity");
  // Three restrained spring-contact lands visible down the barrel.
  for (const [i, angle] of [0.2, 2.3, 4.4].entries()) {
    yield* add(`device-headphone-contact-${i}`, roundedRectShape(1.5, 2.8, 0.5),
  ...
  yield* add("device-dock-collar", roundFrame(dock.width, dock.depth, dock.radius,
    dock.innerWidth, dock.innerDepth, dock.innerRadius), dock.x, dock.z, -1, -1.2, 0.1, "metal");
  yield* add("device-dock-reveal", roundFrame(dock.innerWidth + 0.8, dock.innerDepth + 0.8, dock.innerRadius + 0.4,
    dock.innerWidth - 1, dock.innerDepth - 1, dock.innerRadius - 0.5), dock.x, dock.z, -1, -dock.cavityDepth, -0.4, "insulator");
  yield* add("device-dock-well", roundedRectShape(dock.innerWidth, dock.innerDepth, dock.innerRadius),
  ...
  yield* add("device-dock-tongue", roundedRectShape(dock.tongueWidth, dock.tongueDepth, 0.8),
    dock.x, dock.z + 1.8, -1, -dock.cavityDepth, -3, "slider");
  for (let i = 0; i < dock.contactCount; i++) {
    yield* add(`device-dock-contact-${i + 1}`, roundedRectShape(1.35, 2.1, 0.3),
      dock.x + (i - (dock.contactCount - 1) / 2) * dock.contactPitch,
      dock.z + 1.1, -1, -3.1, -2.8, "contact");
  }
  for (const side of [-1, 1]) {
    yield* add(`device-dock-key-${side}`, roundedRectShape(2, 7, 0.4),
