...
try{for(const center of centers)for(const art of arts)for(const width of[.08,.35])for(const rotation of[-137,0,27,45,90,173]){
 ...
 try{const w=330*width,h=w*aspect(art),angle=rotation*Math.PI/180,cos=Math.cos(angle),sin=Math.sin(angle),cage=wrap.cornerCage(center.x,center.y,{width:w,height:h,angle}),points:Vector3[]=[],material:{x:number;y:number}[]=[];let finite=true,zero=0,opposed=0,minArea=Infinity;
