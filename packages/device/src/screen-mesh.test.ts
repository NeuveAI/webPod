...
function makeHandle() {
  const size = { width: DEVICE_LAYOUT.screen.width, height: DEVICE_LAYOUT.screen.height }
  const defaultMaterial = new MeshBasicMaterial({ color: '#0B0D11', toneMapped: false })
  const mesh = new Mesh(new PlaneGeometry(size.width, size.height), defaultMaterial)
  const camera = new PerspectiveCamera(30, VIEWPORT.width / VIEWPORT.height, 100, 3000)
...
describe('the screen mesh boundary', () => {
  ...
  test('viewport corners are canvas CSS px with +y DOWN', () => {
    ...
    expect(viewport.corners.topLeft.y).toBeLessThan(viewport.corners.bottomLeft.y)
    expect(viewport.corners.topLeft.x).toBeLessThan(viewport.corners.topRight.x)
    ...
    expect(viewport.corners.topLeft.x + viewport.corners.topRight.x).toBeCloseTo(400, 6)
    expect(viewport.corners.topLeft.y + viewport.corners.bottomLeft.y).toBeCloseTo(800, 6)
  })
  ...
  test('a moved device reports a new transform, not the one it was created with', () => {
    ...
    expect(after.viewport.corners.topLeft.y).toBeLessThan(before.viewport.corners.topLeft.y)
  })
  ...
  test('onTransformChange delivers the value and fires only on a real change', () => {
    ...
    mesh.onBeforeRender(...([] as unknown as Parameters<Mesh['onBeforeRender']>))
    expect(seen).toEqual([102])
