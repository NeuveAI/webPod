...
const CALLS: readonly (readonly [string, string, (p: MusicProvider) => unknown])[] = [
  ...
  ['relatedTracks', 'libraryRead', (p) => p.relatedTracks(PLAYLIST)],
  ['relatedAlbums', 'libraryRead', (p) => p.relatedAlbums({ kind: 'artist', key: TRACK.key, provider: 'fixture', catalogId: 'artist', name: 'Artist' })],
