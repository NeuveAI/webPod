...
describe('native sticker tool contracts', () => {
  test('nine discoverable tools route to live controls with accurate read hints', async () => {
    const f = fixture()
    expect(new Set(f.tools.map(tool => tool.name)).size).toBe(9)
    ...
    await f.execute('rotate_sticker', { degrees: -90 }); await f.execute('add_sticker_wear', { amount: .2 }); await f.execute('place_sticker', { x: .5, y: .2 })
    expect(f.calls).toEqual(['open', 'close', 'previous', ['PW-A01', 'placed'], 'release', -90, .2, [.5, .2]])
  })
  test('runtime rejects malformed fields, nonfinite values and range errors before invoking', async () => {
    const f = fixture()
    for (const [name, input] of [['open_sticker_pack', { extra: 1 }], ['sticker_list', null], ['get_sticker', { stickerId: '', source: 'placed' }], ['get_sticker', { stickerId: 'PW-A01', source: 'other' }], ['navigate_sticker_collection', { direction: 'back' }], ['rotate_sticker', { degrees: Infinity }], ['rotate_sticker', { degrees: 361 }], ['add_sticker_wear', { amount: -.1 }], ['add_sticker_wear', { amount: NaN }], ['add_sticker_wear', { amount: 2 }], ['place_sticker', { x: -1, y: .5 }], ['place_sticker', { x: .5 }]] as const) await expect(f.execute(name, input)).rejects.toThrow()
