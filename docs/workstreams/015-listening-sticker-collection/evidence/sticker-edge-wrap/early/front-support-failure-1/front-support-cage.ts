...
export function createStickerCornerCage(
  ...
  height: (x: number, y: number) => number | null, halfDiagonal: number,
) {
  const { width, height: bodyHeight, cornerR } = DEVICE_LAYOUT.body;
  if (![centerX, centerY, halfDiagonal].every(Number.isFinite) || halfDiagonal < 0) throw new Error('Invalid bend footprint');
  ...
  for (const r of [rearRadius, cornerR, cornerR - form.seamWidth - form.frontBevel]) {
    ...
    for (const offset of [-halfDiagonal, 0, halfDiagonal]) {
  ...
  const angle = baseAngle + (lowAngle + highAngle) / 2, nx = Math.cos(angle), ny = Math.sin(angle);
  ...
  const mapped = (depth: number, arc: number) => {
    if (depth > frontEnd) {
      ...
      if (z === null) throw new Error(JSON.stringify({error:'Bend has no actual front support',x,y,depth,arc,frontEnd,frontRadius,travel,N:[nx,ny],rim:p,center:[centerX,centerY]}));
  ...
  const rawPosition = (dx: number, dy: number) => {
    ...
    let low = origin + Math.min(linearDepth, q.depth), high = origin + Math.max(linearDepth, q.depth);
