...
try{
 ...
 const results=await Promise.allSettled(leaves.map(async leaf=>{phase=`parallel-packet:${leaf.id}`;const resource=await preparePackGeometry(leaf.geometry,new AbortController().signal);geometryOwners.push(resource.release);const geometry=resource.parts.geometry;assert(geometry);await print(leaf.artId,geometry);}));
...
const result={carryWorkers,workerErrors,phase,failure,preparedPrints:owners.length,stages,finalTransactions:inspectStickerTransactions(),finalPaper:inspectPaperPool(),scope:'Actual preparePrint five-art warmup then parallel8packet, exact catalogue alpha/authored packet; no mounted React scheduling claim'};
await Bun.write(new URL('./gl-cold-parallel-workload.json',import.meta.url),JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify({carryWorkers,workerErrors,phase,failure,prepared:owners.length,finalTransactions:result.finalTransactions}));await GlobalRegistrator.unregister();
