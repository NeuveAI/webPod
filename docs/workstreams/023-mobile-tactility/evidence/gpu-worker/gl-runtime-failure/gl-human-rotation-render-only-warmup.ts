...
const print=async(id:string,geometry:BufferGeometry)=>{const value=await preparePrint({id,texture:await textureFor(id),geometry,wearGeometry:geometry,wear:0},new AbortController().signal,{prepareContour:!phase.startsWith('warmup:')});
 if(phase.startsWith('warmup:')){
  const descriptor=preparedStickerResources(value.geometry).find(d=>d.input.kind==='damage');assert(descriptor);
  const channel=new MessageChannel();
  const received=new Promise<Uint8Array>(resolve=>{channel.port2.onmessage=event=>{assert.equal(event.data.result.kind,'render-damage');resolve(event.data.result.gpu);channel.port2.close();};channel.port2.start();});
  const lease=await acquirePrivateStickerTransaction(descriptor.key,descriptor.input,channel.port1,new AbortController().signal,{purpose:'render-damage'});
  const gpu=await received;assert.deepEqual(gpu,value.damage.texture.image.data);
  geometryOwners.push(()=>{assert(gpu.byteLength>0);lease.release();});value.release();
 }else owners.push(value);stages.push({phase,transactions:inspectStickerTransactions()});};
try{
 ...
 try{
  ...
  await act(async()=>root.render(createElement(Probe,{geometry:surface.geometry,epoch:6})));await wait(()=>displayed!==before);assert.equal(read().damage.field,before.damage.field);
