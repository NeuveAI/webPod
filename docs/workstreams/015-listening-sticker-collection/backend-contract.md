...
Import snapshots remain all-or-nothing for authorization, upstream errors and malformed pages: preserve the previous taste/grants, mark failure and retry the bounded snapshot. The owner's 24-page timeout report established a narrow policy correction: expiry of the importer's own 30-second sampling budget may retain fully validated usable pages as `partial`, with `sample_time_limit` diagnostics. External request/logout/runtime cancellation still prevents writes, and own-budget expiry without a usable sample remains a failure. `partial` also reports the unchanged 25-page/2,500-track cap. It never disguises an independent network or validation error. See import-budget-repair.md.
