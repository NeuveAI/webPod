/**
 * The canonical-pose acceptance criterion, as code.
 *
 ...
 *
 * ⚑ D-064 is load-bearing here: these targets are the **front and rear
 * reference-pose** targets only. They are not a pose-wide pixel contract, and
 * they must not be re-applied unchanged to three-quarter, edge, or animated
 * poses. Rotated views are validated by physical continuity, world-fixed
 * lighting, material identity, silhouette, and occlusion instead.
 *
 ...
 * stop 3 live, and what value should it read in the canonical reference
 * views?* — and grades the answers a caller brings back. Keeping it
 * renderer-free is what lets the same targets be checked against a WebGL
 * readback today and against something else later, and it keeps the gate from
 * sharing an implementation with the thing it gates.
 *
