...
import { fakeMusic, setup } from './test-fixtures'
...
describe('Apple provider', () => {
  ...
  test('configures, authorizes, signs out, and reports storefront', async () => { const { provider, music } = setup(); const sessions: unknown[] = []; const off = provider.onSessionChange((value) => sessions.push(value)); await provider.configure(); const session = await provider.authorize(); expect(session.storefront).toBe('se'); expect(session.userIdentifier).toBeNull(); expect(JSON.stringify(sessions)).not.toContain('opaque-user'); expect(await provider.withMusicAuthorization(async (token) => token === 'opaque-user')).toBe(true); await provider.unauthorize(); off(); expect(sessions.at(-1)).toBeNull(); expect(music.calls).toContain('unauthorize') })
...
test('Apple artist albums publish pages before the full request completes', async () => {
  const { provider, music } = setup()
  await provider.configure()
  const playlist = (await provider.libraryList('playlists')).items[0]
