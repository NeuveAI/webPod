...
const print=async(id:string,geometry:BufferGeometry)=>{owners.push(await preparePrint({id,texture:await textureFor(id),geometry,wearGeometry:geometry,wear:0},new AbortController().signal,{prepareContour:!phase.startsWith('warmup:')}));stages.push({phase,transactions:inspectStickerTransactions()});};
try{
