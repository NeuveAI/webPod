...
describe('the vertical chain, re-derived at wheelR 115', () => {
  ...
  test('48 + 204 + 43 + 230 + 27 = 552 (§7.3 millimetres, §12.0 wheel)', () => {
    ...
    expect(chain.topToGlass + 204 + chain.glassToWheel + 230 + chain.bottomMargin).toBe(552)
  })
  ...
  test('the bottom margin lands on §7.3’s measured 5.2mm', () => {
    ...
    expect(Math.abs(DEVICE_LAYOUT.chain.bottomMargin - 5.2 * PX_PER_MM)).toBeLessThan(1)
  })
  ...
  test('screen centre sits 126px above the body centre', () => {
    ...
    expect(toCanvasTopLeft(0, DEVICE_LAYOUT.screen.centerY)).toEqual({ x: 165, y: 150 })
  })
  ...
  test('wheel centre sits 134px below the body centre', () => {
    ...
    expect(toCanvasTopLeft(0, DEVICE_LAYOUT.wheel.centerY)).toEqual({ x: 165, y: 410 })
  })
