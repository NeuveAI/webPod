...
import {pngAlpha} from '/tmp/webpod-png-alpha';
...
const root='/Users/vinicius/code/webPod/',dir=root+'docs/workstreams/015-listening-sticker-collection/evidence/sticker-edge-wrap/early/front-residual-1-native/',out=root+'docs/workstreams/015-listening-sticker-collection/evidence/sticker-edge-wrap/early/wrapped-witness-1/';
...
const results=[]; const failures=[];for(const viewport of[1280,375])for(const face of['side','front']){
 ...
 if(distance>hit.distance+.01){hidden.push(item);continue;}if(item.facing<.15||hit.distance-distance>.5)continue;
 ...
 const dx=release.x-pickup.point.x,dy=release.y-pickup.point.y,length=Math.hypot(dx,dy),partial={x:pickup.point.x+dx/length*12,y:pickup.point.y+dy/length*12};const grab=captureStickerSurfaceGrab(placement,art,new Vector2(...pickup.uv),pickup.point,content,camera,{getBoundingClientRect:()=>rect},wrap,()=>true);if(!grab)continue;
 ...
 results.push({viewport:capture.viewport,face,pose:capture.pose,build:capture.build,bufferHashes,drawIds:candidates.map(c=>c.draw.id),matrices:{cameraWorld:cameraWorld.toArray(),cameraProjection:projection.toArray(),contentWorld:world.toArray()},chosen,visibleCount:visible.length,hiddenCount:hidden.length,hidden:hidden[0],attached:visible.filter(v=>v.row>.24).sort((a,b)=>b.row-a.row)[0],visible:visible.slice(0,10),capturePath:path,captureSha256:hash(await Bun.file(path).text()),rect,shellDrawCorrelation:faceBuffers.map(f=>({...f,draws:capture.submitted.recentDraws.filter(d=>d.bindings.position?.fnv1a===f.position.fnv1a&&d.bindings.position?.byteLength===f.position.byteLength).map(d=>({id:d.id,pass:d.pass,modelMatrix:d.matrices.modelMatrix,position:d.bindings.position}))}))});mesh.material.dispose();
