...
import { setPreparedStickerContour } from './sticker-contour-preparation-data';
...
interface PreparedPrint { readonly texture: Texture; readonly geometry: BufferGeometry; readonly damage: StickerDamageResource; readonly wear: number; release(): void }
interface PrintInput { readonly texture: Texture; readonly id: string; readonly geometry: BufferGeometry; readonly wearGeometry: BufferGeometry | null; readonly wear: number; readonly onError?: (id: string) => void }
...
async function preparePrint(input: PrintInput, signal: AbortSignal): Promise<PreparedPrint> {
  ...
  try {
    ...
    const contour = await requestStickerTransaction(contourKey, contourInput, signal); releases.push(contour.release);
    if (contour.value.kind !== 'contour') throw new Error('Unexpected sticker contour result');
    ...
    setPreparedStickerContour(own, damage.value.field, wear, contour.value.contour);
