import { Mesh, MeshBasicMaterial, Raycaster, Vector3, Euler, ExtrudeGeometry, type Intersection } from '../../../../../packages/device/node_modules/three';
import { toCreasedNormals } from '../../../../../packages/device/node_modules/three/examples/jsm/utils/BufferGeometryUtils.js';
import { createOrientationRaycast } from '../../../../../packages/device/src/orientation-picking';
...
import { isDeviceOuterGrabPoint } from '../../../../../packages/device/src/orientation-grab';
...
const report=[];
for(const [name,geometry] of [['front',front],['rear',rear]] as const) for(const touch of [false,true]) {
 const shell=new Mesh(geometry,new MeshBasicMaterial()); const proxy=new Mesh(geometry,shell.material);
 const original=shell.raycast;let exactCalls=0;
 shell.raycast=function(ray,hits){exactCalls++; original.call(this,ray,hits)};
 proxy.raycast=createOrientationRaycast(()=>shell,()=>touch,completeDeviceEnvelope());
 let baselineMs=0,optimizedMs=0,checked=0,rejected=0;
 for(const [pitch,yaw] of poses) {
  proxy.rotation.copy(new Euler(pitch!*Math.PI/180,yaw!*Math.PI/180,0));proxy.updateMatrixWorld();shell.rotation.copy(proxy.rotation);shell.updateMatrixWorld();
  for(const [x,y] of points) {
   const origin=new Vector3(0,0,1500), target=new Vector3(x,y,0).applyMatrix4(proxy.matrixWorld); const ray=new Raycaster(origin,target.sub(origin).normalize());
   const expected:Intersection[]=[],actual:Intersection[]=[];
   let started=performance.now();original.call(shell,ray,expected);baselineMs+=performance.now()-started;
   const before=exactCalls;started=performance.now();proxy.raycast(ray,actual);optimizedMs+=performance.now()-started;
   if(exactCalls===before) {
    rejected++;
    if(expected.some(hit=>{const p=shell.worldToLocal(hit.point.clone());return isDeviceOuterGrabPoint(p.x,p.y,touch?body.width/4:18)}))throw Error('lost perimeter contact');
   } else {
    if(actual.length!==expected.length||actual.some((hit,i)=>hit.point.distanceTo(expected[i]!.point)>1e-8||hit.faceIndex!==expected[i]!.faceIndex||hit.object!==proxy))throw Error('changed exact ray result');
   }
   checked++;
  }
 }
 report.push({name,touch,triangles:(geometry.index?.count??geometry.getAttribute('position').count)/3,rays:checked,exactCalls,broadPhaseRejected:rejected,baselineMs,optimizedMs});
...
for(const [name,geometry] of [['front',front],['rear',rear]] as const) {
 ...
 for(const [pitch,yaw] of poses) {shell.rotation.set(pitch!*Math.PI/180,yaw!*Math.PI/180,0);shell.updateMatrixWorld();
 for(const [x,y] of points) {
  ...
  const expected:Intersection[]=[],actual:Intersection[]=[];
  let t=performance.now();Mesh.prototype.raycast.call(shell,ray,expected);baselineMs+=performance.now()-t;
  t=performance.now();prepared.raycast.call(shell,ray,actual);indexedMs+=performance.now()-t;
  if(expected.length!==actual.length || actual.some((h,i)=> {const e=expected[i]!;return h.faceIndex!==e.faceIndex||h.object!==shell||h.point.distanceTo(e.point)>1e-8||Math.abs(h.distance-e.distance)>1e-8||JSON.stringify(h.face)!==JSON.stringify(e.face)||JSON.stringify(h.uv)!==JSON.stringify(e.uv)||JSON.stringify(h.normal)!==JSON.stringify(e.normal)}))throw Error('indexed parity '+name+' '+pitch+' '+yaw+' '+x+' '+y);
  rays++;
 }}}
 indexReport.push({name,buildMs,rays,baselineMs,indexedMs});prepared.dispose();shell.material.dispose();
}
