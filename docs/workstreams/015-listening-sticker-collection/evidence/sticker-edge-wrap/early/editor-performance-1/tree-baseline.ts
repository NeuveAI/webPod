export let treeHash="", treeBuildMs=0;
 ...
 */
export function createStickerCollision(faces: readonly StickerCollisionFace[]) { const buildStarted=performance.now();
  ...
  treeBuildMs=performance.now()-buildStarted; const digest=new Bun.CryptoHasher('sha256'); const hashNode=(node:Node)=>{digest.update(JSON.stringify([node.box.min.toArray(),node.box.max.toArray(),node.triangles??null]));if(node.left)hashNode(node.left);if(node.right)hashNode(node.right);};if(root)hashNode(root);treeHash=digest.digest('hex');
