...
for(let i=0;i<10;i++){
 ...
 for(const mode of i%2?['cached','baseline']:['baseline','cached']){
  ...
  outputs[mode]=projectStickerContourField(print,camera,r.rect,field,wear,p=>{samples++;return query(p);},mode==='cached'?{identity:vis,revision:vis.revision,contentMatrix:content.matrixWorld.elements}:undefined);
 ...
 const repeats=[];for(let j=0;j<10;j++){let rays=0;const start=performance.now(),query=stickerVisibilityQuery(vis,content,camera),result=projectStickerContourField(print,camera,r.rect,field,wear,p=>{rays++;return query(p);},{identity:vis,revision:vis.revision,contentMatrix:content.matrixWorld.elements});if(result!==outputs.cached)throw Error('Cache object mismatch');repeats.push({ms:performance.now()-start,rays});}rows.push({i,wear,rotationDeg,geometryMs,alphaMs,...timings,repeats});g.dispose();
}
...
await Bun.write(import.meta.dir+'/query-cache.json',JSON.stringify(result,null,2));console.log(JSON.stringify(result));baselineVis.dispose();vis.dispose();material.dispose();shell.dispose();
